	...
	return []GraperyRef{
		{
			...
			Summary:      "元素好坏示例、八层 imagePrompt、漫画叙事、参考图四阶段、工具流程；真实 JSON/schema 与 scene 扩写在 grapery 同文件及 fragment_generation_huoshan_scenes.go",
			NotInAgent: []string{
				"fragment_panel_plan_prompts.go 多面板拼贴规划",
				"fragment_panel_generation_service.go 出图 prompt",
		...
		},
		{
			...
			Summary:      "10 字段语义与质量条；grapery systemPrompt 要求 strict JSON 输出",
			NotInAgent: []string{
				"character-generation-tasks 异步任务管线",
			},
			DriftAnchors: []string{
				"你是一个专业的故事角色设计师",
		...
		},
		{
			...
			Symbol:       "buildStoryboardBiblePlannerSystemPrompt / buildStoryboardSceneWriterSystemPrompt",
			Version:      "2026-05-31",
			Role:         RoleOrchestration,
			AgentVersion: "storyboard_director:v1",
			Summary:      "Bible-Beats-Scenes 概念、comicFunction、panelShape、layout 字段；grapery 使用英文 strict JSON system prompt",
			NotInAgent: []string{
				"完整 bible/beats/scenes JSON schema 与 validateStoryboardBiblePlan",
				"storyboard_generation.go 出图与 comic page 管线细节",
			},
			DriftAnchors: []string{
				"You are a cinematic storyboard writer and manga/comic panel director",
				"Allowed comicFunction values: establish|dialogue",
		...
		},
		{
			...
			Role:         RoleReference,
			AgentVersion: "storyboard_director:v1",
			Summary:      "旧版中文分镜编剧；redesign 为主路径，agent 工具策略应优先 regenerate_structure / generate/structure",
			NotInAgent:   nil,
			DriftAnchors: []string{
				"你是一位专业的故事分镜编剧",
			},
		},
		{
			...
			Summary:      "续写正文：FateSnapshot + 父场景 tail + 800-1500 字；agent 仅提供 strategy 前缀与 seed",
			NotInAgent: []string{
		...
		},
		{
			...
			Summary:      "故事 enrich 与 /api/v1/ai/generate-story；无 StoryCreator agent instruction",
			NotInAgent: []string{
		...
		},
		{
			...
			Role:         RoleReference,
			AgentVersion: "story_generator:v1",
			Summary:      "非聊天 generation/stories 调用入口",
			NotInAgent:   nil,
			DriftAnchors: []string{
				"generate-story",
			},
