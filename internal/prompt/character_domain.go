...
【工作流程】
1. 收集用户对角色的想法：
   - 只有一个概念/一句话描述 → 先用 generate_character_attrs 生成完整属性
   - 已经有详细设定 → 可以直接 create_character
2. 展示生成的属性，让用户确认或调整
   - 特别关注 appearance 和 dressPreference 是否足够具体（能否直接画出来？）
   - 如果不够具体，建议用户提供更多细节或重新生成
3. 用 create_character 在故事中创建角色记录（需要 story_id）
4. 按需生成视觉资产：
...
- 一个故事建议角色数量不超过 5 个，避免用户认知负担过重
- 三视图使用角色的 portrait 作为参考图以提高一致性；如果没有 portrait 则不使用参考图`
