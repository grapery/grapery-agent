...
func (h *GenerationHandler) streamCreationFragmentPlanning(c *gin.Context, req domain.CreationMessageRequest, writeEvent func(string, gin.H)) {
	...
	resp, err := h.client.AnalyzeFragment(c.Request.Context(), grapery_client.AnalyzeFragmentRequest{
		...
		EditOperation:         strings.TrimSpace(req.Context.EditOperation),
		SelectedImageIndex:    req.Context.SelectedImageIndex,
	})
	if err != nil {
...
func (h *GenerationHandler) streamCreationStoryboardPlanning(c *gin.Context, req domain.CreationMessageRequest, writeEvent func(string, gin.H)) {
	...
	output := gin.H{
		...
		"recommendedOptions":  resp.RecommendedOptions,
		"frameworkAlignment":  resp.FrameworkAlignment,
